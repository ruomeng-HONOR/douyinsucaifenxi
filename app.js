...
function renderMaterials() {
  ...
  $("#view-materials").innerHTML = `<section class="tier-grid">
    ...
    <section class="workspace"><div class="table-panel"><div class="table-head"><div class="toolbar"><div><div class="score-title"><b>素材评分明细</b><span class="score-help"><button type="button" class="score-help-button" aria-describedby="score-definition">评分定义 <i>?</i></button><span id="score-definition" class="score-tooltip" role="tooltip"><strong>素材评分口径（0–100分）</strong><span><b>分级：</b>优质 ≥ 70分；普通 40–69分；劣质 ＜ 40分。</span><span><b>样本门槛：</b>所选周期消耗不足 ¥50 时记为0分，不进入稳定评分。</span><span><b>相对评价：</b>推商品与推直播分别在同类素材中排名，日期、店铺或账户改变后会重新计算。</span><span><b>核心指标：</b>贝叶斯整体/净ROI、点击率、转化率、净成交、消耗、退款质量、完播率及3秒/5秒留存；使用 ¥200 先验权重降低小样本波动。</span><span><b>内容质量分：</b>先将完播率、3秒播放率和5秒播放率转换为同类素材百分位；推商品三项各占约33.3%，推直播分别占40%、30%、30%。该分数反映相对留存水平，不等同于实际完播率；消耗不足 ¥50 时为0分。</span></span></span></div><p>点击素材查看投放趋势</p></div><div class="toolbar-group"><label class="field">等级<select id="tier-filter"><option value="all">全部等级</option><option value="good">优质</option><option value="normal">普通</option><option value="poor">劣质</option></select></label><label class="field">素材ID或名称<input id="material-search" placeholder="输入素材ID或名称"></label></div></div></div>
